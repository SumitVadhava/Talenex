...
//       //   if (filters.category === "python") {
//       //     const hasPython = skill.offeredSkills?.some(
//       //       (s) =>
//       //         s.title?.toLowerCase().includes("python") ||
//       //         s.category?.toLowerCase().includes("python")
//       //     );
//       //     if (!hasPython) return false;
//       //   } else if (filters.category === "java") {
//       //     const hasJava = skill.offeredSkills?.some(
//       //       (s) =>
//       //         s.title?.toLowerCase().includes("java") ||
//       //         s.category?.toLowerCase().includes("java")
//       //     );
...
//       //     ) {
//       //       // Also check array of offeredSKills for category match
//       //       const categoryMatch = skill.offeredSkills?.some(
//       //         (s) => s.category === canonicalName
//       //       );
//       //       if (!categoryMatch && skill.offerCategory !== canonicalName) {
//       //         return false;
//       //       }
//       //     }
//       //   }
...
//         if (!q) return true; // strict check

//         // Check Name
//         const nameMatch = skill.user.name?.toLowerCase().includes(q);

//         // Check Offers (Title, Category, Description)
...
//         if (!offerMatch && !seekMatch && !nameMatch) return false;
//       }
...
// Map category ids to match terms for dynamic filtering (to handle variations like "UI/UX" vs "Design")
  ...
  const filteredData = useMemo(() => {
    return skills.filter((skill) => {
      ...
      if (filters.level.length > 0) {
        const selectedLevels = filters.level.map(l => l.toLowerCase()); // make case-insensitive

        const hasMatchingLevel = skill.offeredSkills?.some((s) => {
          const skillLevel = (s.level || "").toLowerCase().trim();
          return selectedLevels.includes(skillLevel);
        });
