      ...
      <div className="p-4 flex justify-between items-start border-b border-slate-50 bg-slate-50/30">
        <div className="flex gap-3">
          <div className="relative">
            ...
            />
            {skill.isOnline && (
              <span className="absolute -bottom-0.5 -right-0.5 block h-2.5 w-2.5 rounded-full bg-green-500 ring-2 ring-white" title="Online" />
            )}
          </div>
          <div>
            <h3 className="text-sm font-semibold text-slate-900 group-hover:text-primary-700 transition-colors">{skill.user.name}</h3>
            <div className="flex items-center gap-1 text-xs text-slate-500 mt-0.5">
              <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
              <span className="font-medium text-slate-700">{skill.user.rating}</span>
        ...
        <div className="flex flex-col items-end gap-1.5">
          {skill.isOnline ? (
            <Badge variant="secondary" className="bg-green-50 text-green-700 border-green-100 text-[10px] px-2 py-0.5 h-auto font-medium">
              <Globe className="w-3 h-3 mr-1" /> Online
            </Badge>
          ) : (
            <Badge variant="outline" className="text-slate-500 border-slate-200 text-[10px] px-2 py-0.5 h-auto font-medium bg-white">
      ...
      <div className="p-5 flex-1 flex flex-col gap-4">
        ...
        <div className="relative">
          ...
          <div className="flex items-center gap-2 mb-1.5">
            <Badge variant="secondary" className="bg-slate-100 text-primary-700 hover:bg-slate-200 border-transparent">Offering</Badge>
          </div>

          <h4 className="font-bold text-slate-900 text-base leading-snug group-hover:text-primary-600 transition-colors">
            {skill.offeredSkills.map(s => s.title).join(" • ")}
          </h4>

          <p className="text-sm text-slate-500 mt-2 leading-relaxed line-clamp-3">
            {skill.offeredSkills.map(s => s.description).filter(Boolean).join(" | ") || "Multiple skills offered"}
          </p>
        </div>

        {/* Seeking Section */}
        <div>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 pl-1">Seeking</p>
